...
```json
{ "type": "...", "from": "...", "to": "...", "conv": "...", "ts": 0, "payload": {} }
```
...
| S→C | `ack` | 通用应答。hello 应答：`{ok, nodeId}`；消息应答：`{ok, clientId, serverId, seq, conv}`；错误：`{ok: false, error}` |
| 双向 | `ping` / `pong` | 心跳（客户端 30s 主动 ping；服务端 15s 扫描） |
| C→S | `msg/send` | `to` = 收件人 nodeId，`payload` = `{clientId, text, ts}` |
| S→C | `msg/push` | 新消息推送，`payload.msg` = `{conv, sender, clientId, text, ts, serverId, seq, hostname}` |
